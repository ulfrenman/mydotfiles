...
beautiful.init(awful.util.getdir("config") .. "/themes/ulf/theme.lua")
...
-- {{{ Tags
-- Modified tag-description from:
--   http://awesome.naquadah.org/wiki/FAQ#How_to_setup_different_tags_and_layouts_per_screen.3F
-- Note that the screens are swapped so that the right-one is specified before
-- the left-one.
tags = {
  settings = {
    { names  = { "im", 2, 3, 4, 5, 6, 7, 8, 9 },
...
ulftest = widget({type = "textbox" })
ulftest.bg = "red"
vicious.register(ulftest, vicious.widgets.date, "%b %d, %R")
...
--cpuwidget:set_max_value(50)
vicious.cache(vicious.widgets.uptime)
cpuwidget_t = awful.tooltip({ objects = { cpuwidget.widget },})

-- Register CPU widget
vicious.register(cpuwidget, vicious.widgets.uptime,
                function (widget, args)
                    -- 1st value as uptime in days, 2nd as uptime in hours, 3rd
                    ...
                    cpuwidget_t:set_text(string.format(
                            'Uptime: %s days, %02d:%02d\n' ..
                            '----------------------------------\n' ..
                            'Load avg 1 minute:     %.2f\n' ..
                            'Load avg 5 minutes:   %.2f\n' ..
                            'Load avg 15 minutes: %.2f',
...
globalkeys = awful.util.table.join(
    ...
    -- Add possibillity to switch screen with mod-ctrl-<left-right>
    ...
    -- Start screensaver (from: http://awesome.naquadah.org/wiki/Xscreensaver)
    awful.key({ "Mod1" , "Control" }, "l",
              function ()
                  awful.util.spawn("xscreensaver-command -lock")
              end),

    -- Spotify (from: http://stackoverflow.com/a/31557802/42580)
